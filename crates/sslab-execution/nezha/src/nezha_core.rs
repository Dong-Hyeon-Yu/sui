...
impl ConcurrencyLevelManager {
    ...
    async fn _validate_optimistic_assumption(
        ...
        if rw_set.len() == 1 {
            let (_, _, state, _, _) = rw_set[0].clone().deconstruct();

            self.global_state.apply_local_effect(state);
...
#[inline]
async fn _convert(txs: Vec<SimulatedTransaction>) -> Vec<ScheduledTransaction> {
    let (send, recv) = tokio::sync::oneshot::channel();
    rayon::spawn(move || {
        let result = txs
            .into_iter()
