...
impl ConcurrencyLevelManager {
    pub fn new(global_state: ConcurrentEVMStorage, concurrency_level: usize) -> Self {
        Self {
            ...
            #[cfg(parallelism_metric)]
            parallelism_metric: Stats::new(),
        }
    }
    ...
    pub async fn _simulate(
        ...
        let tx_list = batches.into_iter().flatten().collect_vec();
        ...
        let result = tokio::task::spawn_blocking(move || {
            tx_list
                .par_iter()
                .filter_map(|tx| match crate::evm_utils::simulate_tx(tx, &snapshot) {
                    Ok(Some((effect, log, rw_set))) => Some(SimulatedTransaction::new(
                        0,
                        tx.digest(),
                        Some(rw_set),
                        effect,
        ...
        match result {
            Ok(rw_sets) => SimulationResult { digests, rw_sets },
            Err(e) => {
                error!(
    ...
    pub async fn _concurrent_commit(&self, scheduled_info: ScheduledInfo, chunk_size: usize) {
        ...
        let _ = tokio::task::spawn_blocking(move || {
            ...
            for txs_to_commit in scheduled_txs {
                txs_to_commit.par_chunks(chunk_size).for_each(|txs| {
...
impl ScheduledInfo {
    pub fn from(
        tx_list: hashbrown::HashMap<H256, Rc<Transaction>>,
        aborted_txs: Vec<Rc<Transaction>>,
    ) -> Self {
        // group by sequence.
        ...
        debug!(
            "{} scheduled transactions are scheduled group by {}",
            count,
            scheduled_txs.len()
        );
        debug!(
